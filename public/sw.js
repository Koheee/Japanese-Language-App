const CACHE_NAME = 'nihongo-path-shell-v1';
const scope = new URL(self.registration.scope);
const local = (path) => new URL(path, scope).toString();
const CORE_FILES = [local('./'), local('manifest.json'), local('icon-192.png'), local('icon-512.png'), local('apple-touch-icon.png')];
    ...
    event.respondWith(
      fetch(request)
        .then((response) => {
          ...
          caches.open(CACHE_NAME).then((cache) => cache.put(local('./'), copy));
          return response;
        })
